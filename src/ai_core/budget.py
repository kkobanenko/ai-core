"""Pure shared-deadline calculations for provider attempts."""
...
class AttemptBudget:
    """Calculate attempt timeouts under one shared request deadline."""
    ...
    def from_timeout(
        ...
        *,
        ...
        return cls(
            deadline_monotonic=(
                now_monotonic + total_timeout_seconds
            ),
    ...
    def remaining_seconds(
        ...
        *,
        ...
        return max(
            0.0,
            self.deadline_monotonic - now_monotonic,
        )
    ...
    def timeout_for_attempt(
        ...
        *,
        ...
        if configured_timeout_seconds <= 0:
            raise ValueError(
                "configured_timeout_seconds must be > 0"
            )

        if future_attempts < 0:
            raise ValueError(
                "future_attempts must be >= 0"
            )

        if reserve_per_future_attempt_seconds < 0:
            raise ValueError(
                "reserve_per_future_attempt_seconds must be >= 0"
            )
        ...
        reserved = (
            future_attempts
            * reserve_per_future_attempt_seconds
        )
        ...
        if available < self.min_attempt_seconds:
            raise RequestDeadlineExceededError(
                "Shared request deadline leaves no safe "
                "budget for another provider attempt"
            )
